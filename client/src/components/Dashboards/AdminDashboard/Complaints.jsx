...
function Complaints() {
  const getComplaints = async () => {
    ...
    const response = await fetch(
      `https://hostel-management-seven.vercel.app/api/complaint/hostel`,
      {
        method: "POST",
        headers: {
  ...
  const dismissComplaint = async (id) => {
    const response = await fetch(
      "https://hostel-management-seven.vercel.app/api/complaint/resolve/",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const graph = (
    <div className="flex items-center justify-center md:h-64 sm:h-40 h-32 md:w-96 w-full">
      <Line
        data={{
          labels: [
            new Date(Date.now() - 6 * 24 * 60 * 60 * 1000).toLocaleDateString(
  ...
  return (
    <div className="w-full min-h-[91vh] flex flex-col gap-10 md:gap-7 pt-32 items-center justify-center overflow-auto">
      <h1 className="text-white font-bold text-3xl lg:text-5xl">Complaints</h1>
