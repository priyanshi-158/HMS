...
const [updatestud,setUpdatestud]=useState({})
  ...
  const openModal=(id)=>{
    setUpdatestud(allStudents.filter((student) => student._id === id)[0])
    setShow(true);
  }
  return (
    <>{show && <Modal2 closeModal={toggleModal} student={updatestud}/>}<div className="w-full flex flex-col gap-3 items-center justify-center min-h-[91vh]">
    <h1 className="text-white font-bold text-3xl lg:text-5xl">All Students</h1>
    <div className="md:w-96 flex justify-center">
      <button
        onClick={getCSV}
        target="_blank"
        download={true}
        className="px-20 py-3 bg-[#9F6BA0] hover:bg-[#9F6BA0] text-white font-bold rounded-xl shadow-xl"
      >
        Download List
      </button>
      <ToastContainer
          ...
          <li className="py-3 px-5 rounded sm:py-4 hover:bg-neutral-700 hover:scale-105 transition-all" key={student._id}>
            <div className="flex items-center space-x-4">
              ...
              <div className="flex gap-3">
                <button className="hover:underline hover:text-green-600 hover:scale-125 transition-all" onClick={()=>openModal(student._id)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
