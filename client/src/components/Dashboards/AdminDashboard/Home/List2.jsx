  ...
  return (
    ...
    >
      <div className="flex flex-col justify-between h-full">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate text-white">
                      {item.student.name} | {item.student.room_no}
                    </p>
                    <p className="text-sm truncate text-gray-400">
                      {item.leaving_date.slice(0,10)} || {item.return_date.slice(0,10)}
                    </p>
                  </div>
                </div>
              </li>
        ...
        <Link className="py-3 text-lg text-center rounded-lg w-full text-white border-[#9F6BA0] border-2 hover:bg-[#825882] transition-all" to={title}>
          Manage {title}
        </Link>
      </div>
    </div>
