...
function Complaints() {
  ...
  const registerComplaint = async (e) => {
    ...
    const res = await fetch("https://hostel-management-seven.vercel.app/api/complaint/register", {
      method: "POST",
      headers: {
  ...
  useEffect(()=> {
    ...
    const fetchComplaints = async () => {
      const res = await fetch("https://hostel-management-seven.vercel.app/api/complaint/student", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="w-full  min-h-[91vh] flex flex-col gap-10 items-center justify-center md:p-0 md:px-10 px-2 overflow-y-auto pt-10">
      <h1 className="text-white font-bold text-3xl lg:text-5xl mt-10">Complaints</h1>
      <div className="flex gap-5 flex-wrap items-center justify-center">
        <form
          method="POST"
          onSubmit={registerComplaint}
          className="md:w-96 w-full py-5 pb-7 px-2 md:px-10 bg-neutral-950 rounded-lg shadow-xl flex flex-col gap-5"
        >
          <div>
            <label
              htmlFor="description"
